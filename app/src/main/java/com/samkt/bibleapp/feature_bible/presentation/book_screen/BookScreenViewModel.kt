            ...
            repository.getAllBooks().onEach {
                when(it){
                    is Resources.Loading ->{
                        _screenState.value = BookScreenState(loading = true)
                    }
                    is Resources.Success ->{
                        _screenState.value = BookScreenState(loading = false, data = it.data ?: emptyList())
                    }
                    is Resources.Error ->{
                        _screenState.value = BookScreenState(errorMessage = it.message)
                    }
                }
